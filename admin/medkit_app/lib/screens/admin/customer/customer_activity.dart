...
class ActivityCostumer extends StatefulWidget {
  static String routeName = "/activitycustomer";
  ActivityCostumer(this.name);
  final String name;
  @override
  State<ActivityCostumer> createState() => _ActivityCostumerState();
}

class _ActivityCostumerState extends State<ActivityCostumer> {
  @override
          ...
          builder: (_, snapshot) {
            if (!snapshot.hasData) {
              ...
              var myItems = snapshot.data!.docs
                  .where((element) => element['uid'] == widget.name);
              return ListView(
                  children: myItems
                      .map(
                        (item) => InkWell(
